...
public interface OrderMapper {
    ...
    @Insert("insert into orderinfo values (#{shipments},#{shipnumber},#{path},#{consignee},#{consnumber},#{itemname},#{ordernumber})")
    ...
    @Update("update orderinfo set path = concat(`path`, '->', #{path}) where `shipments` = #{shipments} and `consignee` = #{consignee} and `ordernumber` =#{ordernumber}")
    int updatePath(@Param("path") String path,@Param("shipments") String shipments,
                   @Param("consignee") String consignee,@Param("ordernumber") String ordernumber);
